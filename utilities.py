...
def updateState_e(State_e):
	...
	State_e.weights_evader[State_e.choice_e] *= math.exp(State_e.estimatedReward[State_e.choice_e] \
		* State_e.gamma_e / numActions) # important that we use estimated reward here!
...
def randomInt(a, b, std, size):
	...
	"""
	assert isinstance(a, int) & isinstance(b, int), "Error: a and b should be two integers!"
	c = (a + b) * 0.5	
	x = np.arange(a, b+1)
	xU, xL = x + 0.5, x - 0.5
