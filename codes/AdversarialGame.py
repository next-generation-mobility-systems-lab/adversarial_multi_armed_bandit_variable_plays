...
from utilities import getAlpha, find_indices, reward, DepRound, randomInt

import os
import sys
sys.path.append(".")
from classes import summary, State
...
def advGame(config,numPlays):
	...
	if config['custom_gamma_pursuer']:
		...
		State_p.gamma =  min([1, math.sqrt(numActions * numActions * numPlays_LB *math.log(numActions/numPlays_UB)\
			 / (( (math.e - 2) * numPlays_UB + numPlays_LB)* numPlays_UB * numRounds ) ) ])

	if config['custom_gamma_evader']:
		State_e.gamma =  min([1, math.sqrt(numActions * math.log(numActions) / ((math.e - 1) *  numRounds ) ) ])
	...
	while True:
		...
		State_p.weights = [w / theSum_pursuer for w in State_p.weights] # normalize the weight vector of the pursuer
		State_e.weights = [w / theSum_evader for w in State_e.weights] # normalize the weight vector of the pursuer

		temp = (1.0 / numPlays - State_p.gamma / numActions) * float( 1.0 / (1.0 - State_p.gamma) )
		...
		assert False in np.isnan(np.array(State_p.probDist)), "Error, probability of pursuer must be a real number"
		assert False in np.isnan(np.array(State_e.probDist)), "Error, probability of evader must be a real number"
		...
		State_p.rewardFull =  [0.0] * numActions # initialize reward vector for the pursuer
		State_e.rewardFull = [0.0] * numActions # initialize reward vector for the evader
		...
		State_p.scaledReward = [(r - rewardMin) / (rewardMax - rewardMin) for r in State_p.rewardFull] # reward vector scaled to 0,1, optional
		State_e.scaledReward = [(r - rewardMin) / (rewardMax - rewardMin) for r in State_e.rewardFull] # reward vector scaled to 0,1, optional
		...
		assert State_p.cumulativeReward + State_e.cumulativeReward == 1, "Error, should be constant sum game!"
					...
					try:
						for (State_p, State_e) in advGame(config_search, summary.numPlays[t]):
							...
							print("pursuer weights:(%s)\tevader weights(%s)\t" % \
								(', '.join(["%.3f" % r for r in distr_multiPlays(State_p.weights, summary.numPlays[t])]),','.join(["%.3f" % r for r in distr(State_e.weights)])))
						...
						for s in range(nR_val):
							...
							summary.regretBound_p.append((1 + (math.e - 2)* config_search['numPlays_UB'] / config_search['numPlays_LB'])* State_p.gamma \
								* summary.bestActionCumulativeReward_p + (config_search['numActions'] * math.log(config_search['numActions']/config_search['numPlays_UB'])) / State_p.gamma)
							
							summary.regretBound_e.append((math.e - 1) * State_e.gamma * summary.bestActionCumulativeReward_e \
								+ (config_search['numActions'] * math.log(config_search['numActions'])) / State_e.gamma) 
