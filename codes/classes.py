...
class State:
	...
	def updateState_p(self):
		...
		for i in range(numActions):
			self.weights[i] *= math.exp(numPlays * self.estimatedReward[i] * self.gamma / numActions) # important that we use estimated reward here!
	...
	def updateState_e(self):
		numActions = len(self.scaledReward)
		self.estimatedReward = [0.0] * numActions
		self.estimatedReward[self.choice] = 1.0 * self.scaledReward[self.choice] / self.probDist[self.choice]
		self.weights[self.choice] *= math.exp(self.estimatedReward[self.choice] \
			* self.gamma / numActions) # important that we use estimated reward here!
